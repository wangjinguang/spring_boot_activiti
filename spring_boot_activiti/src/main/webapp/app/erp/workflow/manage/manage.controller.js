...
function WorkflowController ($scope,$http,$state,layer,Upload) {
    ...
    var colModel=[
        ...
        { field: 'refundOrderCode', displayName: '退单号',type: 'text', width: '10%'},
    ...
    var runing_colModel=[
        ...
        { field: 'refundOrderCode', displayName: '单号',type: 'text', width: '5%',cellTooltip: true}
    ];
    ...
    $scope.start=function(row){
        var result =  $http({
            ...
            params:{
                key:row.entity.key,
                refundOrderCode: row.entity.refundOrderCode
            }
        });
        result.then(function(response){
            var data=response.data;
